...
import torch.optim
from .module import compute_loss, eval_errors, compute_sc_loss
...
class Tandem(pl.LightningModule):
    def __init__(self, hparams: dict):
        ...
        self.cva_mvsnet = CvaMVSNet(
            ...
            normal_sparse=self.hparams.get('TRAIN.NS', False),
        )
    ...
    def forward(self, batch: dict, stage1_depth=None):
        ...
        outputs = self.cva_mvsnet(
            image=batch['image'],
            sparse=batch['sparse'] if self.use_sparse else None,
            confidence=batch['confidence'] if self.use_sparse else None,
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.current_epoch < 10 and self.use_sparse and self.cva_mvsnet.sc2dc:
        ##############################################
            gt_depth = batch['depths_out']
            if self.hparams['TRAIN.NS']:
                gt_depth /= 250
            sc_loss = compute_sc_loss(outputs["coarse_depth"], gt_depth, self.current_epoch+1)
            loss = loss + 0.1 * sc_loss
            losses.update({'sc_loss': sc_loss})
        #################################################
