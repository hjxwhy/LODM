...
def generate_depth_map(camera, Xw, shape):
    ...
    Xc = camera.p_cw_aligned * Xw
    Xc = np.matmul(camera.K, Xc.T).T

    # Project the points as if they were in the camera's frame of reference
    uv = camera.project(Xw).astype(int)
...
class Camera:
    ...
    def __init__(self, K, D=None, p_cw=None, align_pose=None):
        ...
        self.p_cw_aligned = Pose()
        self.align_pose = align_pose
        if align_pose is not None:
            R_s = np.matmul(align_pose.rotation_matrix, self.p_cw.inverse().rotation_matrix)
            t_s = align_pose.scale * np.matmul(align_pose.rotation_matrix, self.p_cw.inverse().tvec) + align_pose.tvec
            self.p_cw_aligned = Pose(wxyz=Quaternion(matrix=R_s), tvec=t_s).inverse()
        assert isinstance(self.K, np.ndarray)
        assert isinstance(self.p_cw, Pose)

    # @property
    def rodrigues(self, align=False):
        ...
        """
        if align:
            R = self.p_cw_aligned.rotation_matrix
    ...
    def project(self, Xw):
        ...
        if self.align_pose is not None:
            uv, _ = cv2.projectPoints(Xw, self.rodrigues(align=True), self.p_cw_aligned.tvec, self.K, self.D.coefficients)
        else:
    ...
    def reconstruct(self, depth, frame='w'):
        ...
        grid = np.stack([xs, ys], axis=0).reshape((2, -1)).transpose(1, 0)
        Xc = self.unproject(grid) * depth.reshape([-1, 1])
        ...
        # If in world frame of reference
            ...
            Xw = self.p_cw.inverse() * Xc
            if self.align_pose is not None:
                Xw = self.align_pose.scale * np.matmul(self.align_pose.rotation_matrix,
                                                       Xw.T).T + self.align_pose.tvec
            return Xw
    ...
    def sample(self, world_points, ref_image, height, width):
        uv = self.project(world_points)
        X = uv[:, 0]
        Y = uv[:, 1]
        Xnorm = 2 * X / (width - 1) - 1
        Ynorm = 2 * Y / (height - 1) - 1
        ref_coords = np.stack([Xnorm, Ynorm], axis=-1).reshape([height, width, 2])
        ref_coords = torch.from_numpy(ref_coords).float().unsqueeze(0)
        ref_image = torch.from_numpy(ref_image).float().unsqueeze(0).permute([0, 3, 1, 2])
        warp_image = func.grid_sample(ref_image, ref_coords, mode='bilinear', padding_mode='zeros', align_corners=True)
        return warp_image.permute([0, 2, 3, 1]).squeeze(0).numpy()
